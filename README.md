...
Запуск начинается с баннера: логотипы, градиентная линия и полоса состояния —
по квадратику `▰` на каждый зарегистрированный инструмент (зелёный `ready`,
красный `blocked`, приглушённый `not installed`), счётчик готовых, платформа и
версия. Видно сразу, готов ли инструментарий, не запуская `status`.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
╭─────────────────────────────────────────────────╮
│  ▰▰▰▰▰▰▰▰   6/8 ready   windows/amd64   v1.0.0  │
╰─────────────────────────────────────────────────╯

   (логотип вуза)
```

Дальше отображается рабочая панель: слева — подсказки и текущие настройки,
справа — таблица инструментов со статусом.
...
### Что происходит под капотом (`-v` / `--verbose`)

Чтобы увидеть, что именно делает команда — какой релиз выбран, что и откуда
качается, куда распаковано, — добавь `-v` (или `--verbose`) к `init`, `run`,
`pipeline` или `status`:

```bash
cyberfw init -v
cyberfw init gowitness --verbose   # видно и шаги скачивания Chromium
```

Флаг поднимает уровень логов до `DEBUG` только для этого запуска (постоянную
настройку `log_level` не меняет) и печатает трассировку установки: разрешение
релиза (`release <repo>: resolved <version>`), загрузку каждого ассета
(`download <url>`), выбранный под платформу файл, каталог распаковки и путь к
найденному бинарнику; для Chromium — платформу Chrome for Testing, версию и
URL. То же самое без флага можно включить через `CYBERFW_LOG_LEVEL=DEBUG` или
`log_level: DEBUG` в `config.local.yaml`. При `log_to_file: true` эти строки
дублируются в `logs/cyberfw.log`.
