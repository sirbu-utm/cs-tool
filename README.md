...
Перед стартом `pipeline` проверяет, что прогон вообще способен дать результат:

- бинарники всех этапов запускаются (не `not installed` и не `blocked`);
- есть то, без чего инструмент не работает вовсе, — например headless
  Chrome/Chromium для `gowitness`.

Если чего-то не хватает, команда называет инструмент, этап и причину и выходит
с кодом 2, не тратя минуты сканирования. Запустить вопреки этому —
`--force-start`: доступные этапы отработают, недоступный будет помечен
`failed`, а зависящие от него — `skipped`.

Необязательные зависимости (`check_deps` в `registry.yaml`, сейчас это `nmap`
для `rustscan`) прогон не останавливают: о них печатается предупреждение с
командой установки, и сканирование идёт дальше — rustscan найдёт открытые
порты, просто без определения версий сервисов.
